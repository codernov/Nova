...
typedef struct {
  uint16_t ro;    // RSSI open
  uint16_t rc;    // RSSI close
  uint8_t no;     // Noise open
  uint8_t nc;     // Noise close
  uint8_t go;     // Glitch open
  uint8_t gc;     // Glitch close
} SQL;

// RSSI to S, tables from https://www.giangrandi.org/electronics/radio/smeter.shtml
static const uint8_t rssi2s[2][15] = {
    {121, 115, 109, 103, 97,   91, 85,  79, 73, 63, 53, 43, 33, 23, 13},    // HF, bellow 30 MHz
    {141, 135, 129, 123, 117, 111, 105, 99, 93, 83, 73, 63, 53, 43, 33}     // VHF, 30 MHz and above
};
